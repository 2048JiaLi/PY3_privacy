...
In \cite{a2}, the SVSM protocol first selected $2k$ itemsets with highest guessing frequencies to construct candidate set $IS$, then utilized SVIM protocol again with the constructed domain $IS$ to mine $top-k$ itemsets.We observe that, the size of candidate set to construct $IS$ increase significantly with $k$. As a result, it is computationally expensive when $k$ is large (e.g., $k=100$). 
...
In the local setting, there is no trusted third party and an aggregator wants to gather information from users, where each user possesses an input $v$ in value domain $I$. The privacy of the data contributor is protected by perturbing her/his original data at the data contributor’s side; thus, the agregator cannot access the original data, but is still able to obtain population statistics.

\textbf{Definition 1 (Local Differential Privacy)} A randomized algorithm $\mathcal{A}$ satisfies $\epsilon$-LDP, if and only if for (1) all pairs of input $v_i,v_j \in I$, and (2) any possible output $\mathcal{O}$ of $\mathcal{A}$, we have:

$$\frac{\mathbf{Pr}[\mathcal{A}(v_i)=\mathcal{O}]}{\mathbf{Pr}[\mathcal{A}(v_j)=\mathcal{O}]} \leq e^{\epsilon}$$\\
where $\epsilon \leq 0$ is the privacy budget (the larger $\epsilon$ is, the stronger the privacy guarantee).
...
\begin{thm}
Given $m$ randomized algorithms $\mathcal{A}_i(1 \leq i \leq m)$, each of which satisfies $\epsilon_i$-differential privacy. Then the sequence of $\mathcal{A}_i$ satisfies $(\sum_{i=1}^{m} \epsilon_i)$-differential privacy. 
\end{thm} 
